...
# import math 
# from fake_bert.models.groupLinear import GroupLinear
# import numpy as np
...
#         #                                     dropout=self.dextra_dropout
#         #                                     )
#         self.input_layer = nn.Embedding(self.in_features, self.in_proj_features)
...
#                                                            layer_config['groups'],
#                                                            groups_next_layer)):
#             print("g_l, dropout=self.dextra_dropout", n_in, n_out, g_l)
#             wt_layer = GroupLinear(n_in, n_out, n_groups=g_l, dropout=self.dextra_dropout)
...
#         #                                      dropout=dextra_dropout
#         #                                      )
#         self.output_layer = nn.Linear(self.out_features + self.in_proj_features, self.out_features)
...
#         mid_point = int(math.ceil(n_layers / 2.0)) #2
#         # decide number of groups per layer
#         groups_per_layer = [min(2 ** (i + 1), max_groups) for i in range(mid_point)] # [2 4]

#         # divide the space linearly between input_features and max_features
#         output_sizes = np.linspace(in_features, max_features, mid_point, dtype=np.int32).tolist() # 40 80 [40 80]
#         # invert lists to get the reduction groups and sizes
#         inv_output_sizes = output_sizes[::-1] # [80 40]
#         inv_group_list = groups_per_layer[::-1] # [4 2]
       
#         if n_layers % 2 == 0:
#             # even
#             groups_per_layer = groups_per_layer + inv_group_list # [2 4 4 2]
#             output_sizes = output_sizes + inv_output_sizes # [40 80 80 40]
#         else:
#             # for odd case,
#             groups_per_layer = groups_per_layer + inv_group_list[1:]
#             output_sizes = output_sizes + inv_output_sizes[1:]
...
#         assert len(output_sizes) == len(groups_per_layer), '{} != {}'.format(len(output_sizes), len(groups_per_layer))
#         output_sizes = output_sizes[:-1] # [40 80 80]

#         # ensure that output and input sizes are divisible by group size
#         input_sizes = [1] * len(groups_per_layer) # [1 1 1 1]
#         input_sizes[0] = in_features # [40 1 1 1]
#         # print("n_layer", n_layers)
#         for i in range(n_layers - 1):
#             # output should be divisible by ith groups as well as i+1th group
#             # Enforcing it to be divisble by 8 so that we can maximize tensor usage
#             g_l = max(groups_per_layer[i + 1], groups_per_layer[i], 8) # [8 8 8]
#             out_dim_l = int(math.ceil(output_sizes[i] / g_l)) * g_l # [40 80 80]
#             inp_dim_l1 = out_dim_l + in_features # [80 120 120]
...
#     # model = DExTraUnit(in_features=self.embed_dim,
...
#     #                                    is_iclr_version=args.define_iclr
#     #                                    )
#     model = DExTraUnit(in_features=128,
#                                        in_proj_features=256,
#                                        out_features=324,
...
#     data_sample = next(iter(dataloader))[0]
